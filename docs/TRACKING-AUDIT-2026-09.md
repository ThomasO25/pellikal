...
| Thank-you page | ~~**None.** Success is shown inline.~~ **SUPERSEDED 14 Sep 2026.** `/thankyou/` now exists. A confirmed Formspree success fires the dataLayer events and then redirects there, the Google Ads lead conversion fires on the `generate_lead` event before that redirect. Nothing fires on a failed submission, and no submitted data is carried to the page. See `LEAD-FLOW.md`. |
