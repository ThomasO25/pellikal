...
| **Google Ads** (`AW-859941989`) | Conversion measurement on `/thankyou/`; remarketing | Advertising | Via GTM | `ad_storage`, `ad_user_data`, `ad_personalization` all `granted` | advertising identifiers when granted; redacted cookieless pings when denied (`ads_data_redaction`) | inside GTM |
